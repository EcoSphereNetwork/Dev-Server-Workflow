...
echo -e "${YELLOW}Kopiere verbesserte Skripte...${NC}"
...
if ! command -v docker &> /dev/null; then
    echo -e "${YELLOW}Docker ist nicht installiert. Möchten Sie es jetzt installieren? (j/n)${NC}"
    ...
    if [[ "$install_docker" =~ ^[Jj]$ ]]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "${RED}❌ Fehler bei der Installation von Docker.${NC}"
        fi
    fi
fi
...
if ! command -v docker-compose &> /dev/null; then
    echo -e "${YELLOW}Docker Compose ist nicht installiert. Möchten Sie es jetzt installieren? (j/n)${NC}"
...
echo -e "${YELLOW}Möchten Sie weitere Komponenten installieren? (j/n)${NC}"
read -r install_components
if [[ "$install_components" =~ ^[Jj]$ ]]; then
    echo -e "${YELLOW}Welche Komponenten möchten Sie installieren?${NC}"
    echo -e "1) Alle Komponenten"
    echo -e "2) n8n"
    echo -e "3) MCP-Server"
    echo -e "4) OpenHands"
    echo -e "5) AppFlowy"
    ...
    case "$component_choice" in
        1)
            echo -e "${YELLOW}Installiere alle Komponenten...${NC}"
            "$CLI_DIR/install_components.sh" all
            ;;
        2)
            echo -e "${YELLOW}Installiere n8n...${NC}"
        ...
        3)
            ...
            ;;
        4)
            echo -e "${YELLOW}Installiere OpenHands...${NC}"
            "$CLI_DIR/install_components.sh" openhands
            ;;
        5)
            echo -e "${YELLOW}Installiere AppFlowy...${NC}"
        ...
        6)
            ...
            ;;
        7)
            echo -e "${YELLOW}Installiere Web-UI...${NC}"
            "$CLI_DIR/install_components.sh" web-ui
            ;;
        0)
            echo -e "${BLUE}Keine Komponenten ausgewählt.${NC}"
            ;;
        *)
...
echo -e "${YELLOW}Möchten Sie API-Schlüssel konfigurieren? (j/n)${NC}"
read -r configure_api_keys
if [[ "$configure_api_keys" =~ ^[Jj]$ ]]; then
    # Anthropic API-Schlüssel
    echo -e "${YELLOW}Bitte geben Sie Ihren Anthropic API-Schlüssel ein (leer lassen, um zu überspringen):${NC}"
    ...
    # GitHub-Token
    echo -e "${YELLOW}Bitte geben Sie Ihr GitHub-Token ein (leer lassen, um zu überspringen):${NC}"
    read -r github_token
    ...
    # n8n API-Schlüssel
    echo -e "${YELLOW}Bitte geben Sie Ihren n8n API-Schlüssel ein (leer lassen, um zu überspringen):${NC}"
    read -r n8n_api_key

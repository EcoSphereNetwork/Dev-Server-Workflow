...
handle_error() {
    ...
    case "$CURRENT_OPERATION" in
        "load_config")
            echo -e "${YELLOW}[ROLLBACK] Reverting to default configuration...${NC}"
            # No specific rollback needed, just exit
            ;;
        "save_config")
            echo -e "${YELLOW}[ROLLBACK] Reverting changes to configuration file...${NC}"
...
load_config() {
    ...
    echo -e "${BLUE}[INFO] Loading configuration from $config_path (type: $config_type)${NC}"

    # Check if file exists
    if [[ ! -f "$config_path" ]]; then
        echo -e "${YELLOW}[WARN] Configuration file $config_path does not exist${NC}"
        return 1
    fi
    ...
    case "$config_type" in
        "env")
            ...
            while IFS='=' read -r key value || [[ -n "$key" ]]; do
                ...
                echo -e "${GREEN}[DEBUG] Loaded $var_name=${value}${NC}"
            done < "$config_path"
            ;;
        "json")
            # Check if jq is installed
            if ! command -v jq &> /dev/null; then
                echo -e "${RED}[ERROR] jq is required for JSON configuration but is not installed${NC}"
                echo -e "${YELLOW}[INFO] Install jq with: apt-get install -y jq${NC}"
                return 1
            fi
        ...
        "yaml")
            ...
            if ! command -v yq &> /dev/null; then
                echo -e "${RED}[ERROR] yq is required for YAML configuration but is not installed${NC}"
                echo -e "${YELLOW}[INFO] Install yq with: pip install yq${NC}"
                return 1
            fi
            ...
            # Extract all keys at the top level
            local keys=$(echo "$json_config" | jq -r 'keys[]')
            
            # Process each key
            for key in $keys; do
                # Get the value
                local value=$(echo "$json_config" | jq -r ".[\"$key\"]")
            ...
            ;;
        *)
            echo -e "${RED}[ERROR] Unsupported configuration type: $config_type${NC}"
            return 1
            ;;
    esac
...
save_config() {
    ...
    echo -e "${BLUE}[INFO] Saving configuration to $config_path (type: $config_type)${NC}"

    # Check if file exists and create it if needed
    if [[ ! -f "$config_path" && "$create_if_missing" == "true" ]]; then
        echo -e "${YELLOW}[WARN] Creating new configuration file: $config_path${NC}"
        touch "$config_path"
    elif [[ ! -f "$config_path" ]]; then
        echo -e "${RED}[ERROR] Configuration file $config_path does not exist${NC}"
        return 1
    fi
    ...
    echo -e "${GREEN}[INFO] Successfully updated ${key}=${value} in $config_path${NC}"
    return 0
}
...
get_config() {
    ...
    # Check if file exists
    if [[ ! -f "$config_path" ]]; then
        echo -e "${YELLOW}[WARN] Configuration file $config_path does not exist${NC}" >&2
        echo "$default_value"
        return 0
    fi
    ...
    case "$config_type" in
        "env")
            ...
            local value=$(grep "^${key}=" "$config_path" | cut -d= -f2- || echo "$default_value")
        ...
        "json")
            # Check if jq is installed
            if ! command -v jq &> /dev/null; then
                echo -e "${RED}[ERROR] jq is required for JSON configuration but is not installed${NC}" >&2
                echo "$default_value"
                return 0
            fi
        ...
        "yaml")
            # Check if yq is installed
            if ! command -v yq &> /dev/null; then
                echo -e "${RED}[ERROR] yq is required for YAML configuration but is not installed${NC}" >&2
                echo "$default_value"
                return 0
            fi
            ...
            ;;
        *)
            echo -e "${RED}[ERROR] Unsupported configuration type: $config_type${NC}" >&2
            echo "$default_value"
            return 0
            ;;
    esac
...
delete_config() {
    ...
    echo -e "${GREEN}[INFO] Successfully deleted ${key} from $config_path${NC}"
    return 0
}
...
load_all_configs() {
    ...
    for json_file in $(find "${BASE_DIR}" -name "*.json" -not -path "*/node_modules/*" -not -path "*/\.*"); do
        ...
        local prefix=$(echo "$filename" | sed 's/\.json$//' | tr '[:lower:]' '[:upper:]' | tr '-' '_')
    ...
    if command -v yq &> /dev/null; then
        for yaml_file in $(find "${BASE_DIR}" -name "*.yaml" -o -name "*.yml" -not -path "*/node_modules/*" -not -path "*/\.*"); do
            ...
            local prefix=$(echo "$filename" | sed 's/\.ya\?ml$//' | tr '[:lower:]' '[:upper:]' | tr '-' '_')
...
main() {
    ...
    case "$command" in
        "load")
            if [[ $# -lt 2 ]]; then
                echo -e "${RED}[ERROR] Usage: $0 load <config_type> <config_path> [prefix]${NC}"
                return 1
            fi
        ...
        "save")
            if [[ $# -lt 4 ]]; then
                echo -e "${RED}[ERROR] Usage: $0 save <config_type> <config_path> <key> <value> [create_if_missing]${NC}"
                return 1
            fi
        ...
        "get")
            if [[ $# -lt 3 ]]; then
                echo -e "${RED}[ERROR] Usage: $0 get <config_type> <config_path> <key> [default_value]${NC}"
                return 1
            fi
        ...
        "list")
            if [[ $# -lt 2 ]]; then
                echo -e "${RED}[ERROR] Usage: $0 list <config_type> <config_path>${NC}"
                return 1
            fi
        ...
        "delete")
            if [[ $# -lt 3 ]]; then
                echo -e "${RED}[ERROR] Usage: $0 delete <config_type> <config_path> <key>${NC}"
                return 1
            fi
        ...
        *)
            ...
            echo -e "Available commands: load, save, get, list, delete, load-all"
            return 1
            ;;
    esac
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    if [[ $# -lt 1 ]]; then
        echo -e "${RED}[ERROR] Usage: $0 <command> [args...]${NC}"
        echo -e "Available commands: load, save, get, list, delete, load-all"
        exit 1
    fi
